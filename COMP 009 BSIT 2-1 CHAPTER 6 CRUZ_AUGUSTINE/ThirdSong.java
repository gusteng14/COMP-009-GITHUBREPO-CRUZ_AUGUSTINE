...
public class ThirdSong extends FirstSong {
	File song;
	...
	public void createFile() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
		song = new File("Toto - Rosanna (Guitar Solo Cover).wav");
		audio = AudioSystem.getAudioInputStream(song);
		clip = AudioSystem.getClip();
	}
	
	public void playSong(Clip clip, AudioInputStream audio, String userInput1, char userInput2) throws LineUnavailableException, IOException {
		clip.open(audio);
		clip.start();
		while(userInput2 != 'S') {
